...
*/
  ...
  if (newUser.password !== newUser.confirmPassword) {
    return res
      .status(400)
      .json({ message: "Passwords does not match. Please Check Again:)" });
  } else if (newUser.password.trim().length < 6) {
    return res
      .status(400)
      .json({ message: "Cannot have a password less than six characters :(" });
  }
          ...
          .then((data) => {
            ...
            return db
              ...
              .then((data) => {
                return res
                  .status(201)
                  .json({ message: "user signed up horray!" });
              })
              .catch((err) => {
                console.log(err);
              });
